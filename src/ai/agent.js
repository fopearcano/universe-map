...
- For the far future (DEEPTIME): when the pilot asks to see the ~50-Gyr universe / deep time / the era-ladder present, call **set_mode** "deeptime". Its galaxies are NOT in search_sky — call **list_deeptime_galaxies** to find anchor names (e.g. Aeon Hearth, or a tag like DG-####) before you plot/focus/enter one. **plot_route**/**focus** a named anchor to fly to it (they auto-route through the deeptime scale — don't force cosmos); **enter_galaxy** <name> drops inside a galaxy's star field, **exit_galaxy** rises back out. **deeptime_routes** shows/hides the ⟿ Ways of the Deep (the colour-coded intergalactic route network); **deeptime_voyage** lists or flies a curated multi-galaxy voyage (e.g. "The Beacon Circuit", "Pilgrimage to the Mouth") — it loads into the NAV COMPUTER ready to ENGAGE. The DEEPTIME view IS that ~50-Gyr present of the era ladder (the Oceanic/Eonic cusp).
