...
export const TOOLS = [
  ...
  { type: 'function', function: { name: 'deeptime_routes', description: 'Show or hide the DEEPTIME "Ways of the Deep" — the colour-coded intergalactic route network fanning out from the home supergalaxy across the cosmic web. Switches to deeptime first.', parameters: { type: 'object', properties: { show: { type: 'boolean', description: 'true to show the network, false to hide it' } } } } },
  { type: 'function', function: { name: 'deeptime_voyage', description: 'In DEEPTIME, list or fly a curated multi-galaxy VOYAGE (e.g. "The Long Spiral In", "The Beacon Circuit", "Pilgrimage to the Mouth", "The Seam-Well Road", "The Refusal March"). With a name → loads it into the NAV COMPUTER (drive set, course plotted, ready to ENGAGE). With no name → lists the voyages.', parameters: { type: 'object', properties: { voyage: { type: 'string', description: 'optional voyage title; omit to list them' } } } } },
...
export async function runTool(app, qtr, name, args = {}) {
  try {
    switch (name) {
      ...
      case 'deeptime_routes': return app.agentDeeptimeRoutes(args.show);
      case 'deeptime_voyage': return app.agentDeeptimeVoyage(args.voyage);
