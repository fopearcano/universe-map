...
const SECTIONS = [
  ...
  ['Navigate & route', [
    ...
    ['ENGAGE', 'flies the route on autopilot — you can still drag / scroll to orbit and zoom around the ship as it flies. The flythrough paces itself to the journey: it takes longer (log-scaled, ~7–75 s) for a longer route and less time on a faster drive, so a Class ω hop next door is a few seconds while a sub-light crawl to the Great Attractor runs over a minute. Because the map is log-compressed, the ship also flies visibly faster through the zoomed-in inner region and slows (~6×) through the vast outer decades — a constant-real-speed feel that conveys the true scale. A blinking reticle marks the tracked point; ▤ track (or T) opens a live panel with its position, heading, speed and route progress. On a descent route it pauses at each galaxy and drops you inside — ▶ continue to fly on.'],
