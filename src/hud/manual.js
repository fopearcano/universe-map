...
const SECTIONS = [
  ['Four scales', [
    ...
    ['DEEPTIME', 'The far-future universe of the QTR canon (⧗ DEEPTIME or key 4) — the cosmos ~50 billion years on, procedurally generated on the <b>same logarithmic scale and Planck-ΛCDM rules as COSMOS</b> (direction exact, radius log-compressed) but re-grown from those rules rather than the real data, because in 50 Gyr the real cosmos has changed. There is no Sun any more, so a merged <b>home supergalaxy</b> (⌂ Aeon Hearth, the descendant of the Local Group) anchors the origin the way Sol anchors Cosmos. It is <b>larger and more structured than Cosmos</b> — the expansion has run on, so the map reaches past the cosmos horizon (×1.6). Built from a real dataset, it threads a gravitationally-shaped <b>cosmic web</b> — a spanning skeleton of filaments (grown by a minimum-spanning tree over ~90 nodes, then cross-linked and walled the way real surveys map it) that <b>curve and sag</b> between clusters rather than running straight — with 150 named, navigable <b>anchor galaxies</b> (each carrying a far-future dossier — era on the Φ ladder, its inhabitant species, the drive-class that reaches it) and a full catalogue of QTR <b>deep-time objects, events & hazards</b>: gold-ringed <b>seam-wells</b> and engineered <b>kindled wells</b>, <b>seam-pearls</b>, cyan <b>information reefs</b>, pulsing <b>beacon cores</b>, violet-ringed <b>Formless mouths</b>, red <b>law-shards</b>, entangled <b>amplitude twins</b>, plus hazard/frontier <b>regions</b> (the Dead-Sea, squalls, the Κ refusal zone) and <b>event</b> markers (crossings, seam-scars, the aeonic edge). Click any of them for its lore, with a ◇ <b>codex</b> link straight to its entry. It is a <b>matrioska</b> (nested) scale: click any galaxy for its facts, then ⛶ <b>enter</b> (or key G) to fly <b>inside its own star field</b> — a "local"-sized database of tens of thousands of stars, generated on demand (the per-galaxy generator is the seam a live data-fetch slots into later). Pick stars inside; ↑ overview (or Esc) rises back out. The top-centre navigator lists the anchor galaxies and drives enter / overview — and it is a <b>movable, hideable</b> panel like the rest (drag its crumb bar, or toggle it from the ⧗ Deep-Time button in the top dock). <b>Voyages & NavCom work here too</b>: select galaxies and ＋ route them, pick a DRIVE and ENGAGE — the NAV COMPUTER reports honest Deep-Time distances and QTR times (a route waypoint on a galaxy can even descend inside it mid-flight), and Solaris.Ai can drive it — "show me the deep-time universe", "take me to Aeon Hearth", "enter Vael Reach".'],
  ]],
  ['Getting around', [
