...
const SECTIONS = [
  ...
  ['Navigate & route', [
    ...
    ['ENGAGE', 'flies the route on autopilot — you can still drag / scroll to orbit and zoom around the ship as it flies. The flythrough is an unhurried preview of a much longer voyage: the nav HUD names the STORY time — the whole journey as lived in the fiction, counting not just transit but the threading of the seam, the bridge crossings, the approach and port cycles. So a civilized bridge voyage really takes months, an Unruh run days, a Tekné dash hours, and a sub-light circuit its true centuries. (Prefer raw drive-transit time instead? Turn off <b>Realistic voyage time</b> in the NAV COMPUTER — a bridge hop then reads its bare minutes and the preview runs shorter.) The preview itself runs ~22–150 s; – / + (or the - / = keys) scale it ⅛×–8× if you want to dwell on the crossing or skip ahead. Because the map is log-compressed, the ship also flies visibly faster through the zoomed-in inner region and slows (~6×) through the vast outer decades — a constant-real-speed feel that conveys the true scale. A blinking reticle marks the tracked point; ▤ track (or T) opens a live panel with its position, heading, speed and route progress. On a descent route it pauses at each galaxy and drops you inside — ▶ continue to fly on.'],
