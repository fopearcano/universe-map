...
import { initGraphics } from './hud/graphics.js';
...
import { initScaleBar } from './hud/scaleBar.js';
...
async function main() {
  ...
  try {
    ...
    const [clusters, structures, atlas, expeditions, tradeRoutes, deeptime] = await Promise.all([
      ...
      fetch(durl('deeptime.json')).then((r) => r.json()).catch(() => null),
    ]);
    extras = { clusters, structures, atlas, expeditions, tradeRoutes, deeptime };
  } catch (e) {
  ...
  initGraphics(app);           // the GRAPHICS tools panel (post-FX, presets, per-layer)
  ...
  initScaleBar(app);           // TRUE-SCALE linear ribbon (cosmos)
